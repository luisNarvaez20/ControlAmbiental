...
const CambiarEstado = ({ usuario, handleChange }) => {
  ...
  const [estadoSeleccionado, setEstadoSeleccionado] = useState(usuario.cuenta.estado);
  const [showConfirmationModal, setShowConfirmationModal] = useState(false);

  const editar = async () => {
    var datos = {
      estado: estadoSeleccionado,
      external_id: usuario.cuenta.external_id,
    };
  ...
  const handleEstadoChange = (event) => {
    ...
    handleChange(event); // Para actualizar el estado en el componente Usuarios
  };
        ...
        <div className="col-12">
          <div className="card">
            <div className="card-body">
              <h4 className="card-title">Editar estado del usuario</h4>
              <form className="form-sample" onSubmit={handleSubmit(handleConfirmation)}>
                ...
                <p className="card-description">Cuenta:
                  <span className="mx-2 badge rounded-pill bg-secondary">{usuario.cuenta.correo}</span></p>
                <div className="row">
